...
define("bkeditor/0.8.0/bkeditor",[],function(require, exports){
	...
	var jQEditor = {
		...
		**/
		create : function(targetObj,callback){
			...
			try{
				...
				this.error.writeError('created position error: '+ex.message,5,'core');
				this.utils.message(jQEditor.lang['createError'],'finish');
			}
			try{
				// 该区域用来存放编辑器需要加载的弹窗
				...
				// 获取编辑区域的document和window对象，添加到配置中，这样才能对相应的编辑区域做出操作和绑定事件，相当重要
				...
				html+= '<link rel="stylesheet" type="text/css" href="'+conf.cSkin.addr+'skin-inner.css">';
				html+= '<!--[if IE]>';
				...
				this.error.writeError('created iframe error: '+ex.message,5,'core');
				this.utils.message(jQEditor.lang['createError'],'finish');
			}
			// 填充工具条区域 不支持同一页面多种风格的工具条
			// 此处必须使用原生的DOM方法加载css文件，不能使用jQuery的append方法加载
			// 主题的变换目前以加载不同css的方法实现，不能满足同一页面多种风格的需求
			if(typeof this.themeList.toolbar[conf.toolTheme] === 'undefined'){
				var styleTag = document.createElement("link");
				styleTag.setAttribute('type', 'text/css');
				styleTag.setAttribute('rel', 'stylesheet');
				styleTag.setAttribute('href', conf.cSkin.addr+conf.toolTheme+'/skin-'+conf.toolTheme+'.css');
				$("head")[0].appendChild(styleTag);
				this.themeList.toolbar[conf.toolTheme] = true;
			}
			var rangeCheck = setInterval(function(){
				if(typeof conf.editWin.getSelection !== 'undefined'){
					if(jQEditor.tool){
						$('[ref="'+conf.id+'"].bke-toolbarbox').html(jQEditor.tool);
						callback();
					}else{
						var toolsrc = conf.cSkin.addr+conf.toolTheme+'/toolbar.json';
						$.getScript(toolsrc,function(){
		...
		**/
		ready : function(callback){
			...
			if(self.state !== 'ready'){
				stateChange = setInterval(function(){
					var coreReady = (self.error.ready &&self.log.ready &&
									self.load.ready && self.utils.ready &&
									self.command.ready &&
									self.EditorEvent.ready && self.EditorHistory.ready && 
									self.EditorHistory.ready &&
									self.addPlugin.ready && self.addUi.ready &&
									self.dialog.ready );
					if(coreReady === true){
						self.state = 'ready';
		...
		message: function(msg) {
			this.curEditor.error(msg, 'green');
		},
	...
	**/
	function Editor(options){
		...
		jQEditor.create(this,function(){
			try{
				...
				self.baseEvent = new jQEditor.EditorEvent(self.config);
				self.baseHistory = new jQEditor.EditorHistory(self.config);
				self.baseFilter = new jQEditor.EditorFilter(self.config);
				self.pluginEnable = jQEditor.configPlugin(self.config);
				self.fillPanel();
				// 为编辑器编辑区域内容赋值
				//var content = '<ol><li style="font-size:16px;"><p style="text-align:center"><span>abcdefghijklmnopqrst</span></p></li><li><p>1<sub>2<em>34</em><strong>abc</strong>5678</sub>90</p></li></ol>';
				var content = self.config.oriHtml;
				// 在初始化时，对原始内容进行全面过滤
				var finalContent = jQEditor.utils.filterInner(self,content);
				self.config.editDom.body.innerHTML = finalContent;
				self.baseFilter.excute('dom');
				//初始化历史记录的第一个值
				self.baseHistory.setFirstHistory(self.config.editDom.body.innerHTML);
				self.state = 'ready';
				jQEditor.editWin = self.config.editWin;
				jQEditor.editDom = self.config.editDom;
	...
	Editor.prototype = {
		...
		**/
		submit : function(){
			try{
				var submitArea = this.options.submitArea;
				var finalHtml = this.config.editDom.body.innerHTML;
				var submitHtml = jQEditor.utils.filterInner(this,finalHtml);
				submitArea.innerHTML = submitHtml;
				// 考虑到会在一个表单中出现多个编辑器，所以return true让提交的浏览器行为进行下去
				// 所以要在表单提交的委托中调用该接口函数，委托中还可以做其他事情（如处理另一个编辑器）
				return true;
			}catch(ex){
				jQEditor.error.writeError('submit error: '+ex.message,5,'core');
				return false;
			}
		},
		/**
	...
	jQEditor.ready(function(){
		// 此处必须使用原生的DOM方法加载css文件，不能使用jQuery的append方法加载
		// 主题的变换目前以加载不同css的方法实现，不能满足同一页面多种风格的需求
		if(jQEditor.config.editTheme && typeof jQEditor.themeList.editArea[jQEditor.config.editTheme] === 'undefined'){
			var styleTag = document.createElement("link");
			styleTag.setAttribute('type', 'text/css');
			styleTag.setAttribute('rel', 'stylesheet');
			styleTag.setAttribute('href', jQEditor.config.cSkin.addr+jQEditor.config.editTheme+'/skin-'+jQEditor.config.editTheme+'.css');
			$("head")[0].appendChild(styleTag);
			jQEditor.themeList.editArea[jQEditor.config.editTheme] = true;
		}
		//jQEditor.trigger('ready');
		$('textarea.bkeditor').each(function(index,target){
			var id = $(target).attr('id'),
				width = $(target).css('width'),
				height = $(target).css('height');
			var timestamp = (new Date()).getTime();
			var oriHtml = $(target).val();
			var submitArea = $(target)[0];
...
(function(E , $){
	...
	var isLog = true,
		...
		isHistory = true;
	var cTools = {
		all : ['codemirror','bold','italic','about','superscript','subscript','baikelink',
			'strikethrough','underline','removaformat','formatmatch',
			'tolowercase','touppercase','undo','redo','cut','h2','h3','blockquote',
			'fontsizemenu','fontfamilymenu','justifyleft','justifycenter','justifyright',
			'pastetotext','pasteword','image','map','preview','cleardoc',
			'reference','inserttime','insertdate','highlightcode','multiimage',
			...
			'removeformat','forecolormenu','source','autoformat','search','replace'],
		base : ['bold','italic','underline','strikethrough','superscript','subscript',
			'lowercase','uppercase','redo','undo','cut','pastetotext','pasteword',
			'specharsmenu','link','backcolor','forecolor','fontsizemenu','fontfamilymenu',
			'insertunorderedlist','insertorderedlist','source','search','replace'
			],
		normal : ['bold','italic','about','superscript','subscript',
			'strikethrough','underline','removaformat','formatmatch',
			'tolowercase','touppercase','redo','undo','cut','h2','h3',
			'fontsizemenu','fontfamilymenu','justifyleft','justifycenter','justifyright',
			'pastetotext','pasteword','image','map','cleardoc',
			'reference','inserttime','insertdate',
			'insertvideo','inserttablemenu','tableprops','combinecells',
			'combinecolafter','combinerowafter','splittocols','splittorows','splittocells',
			'insertcolbefore','insertcolafter','insertrowbefore','insertrowafter',
			'deleterow','deletecol','deletetable',
			'specharsmenu','link','backcolor','forecolor',
			'linespacemenu','spacebottommenu','spacetopmenu',
			'outdent','indent','insertunorderedlist','insertorderedlist',
			'forecolormenu','source','search','replace'],
		mini : ['bold','italic','underline','tolowercase','touppercase','fontsizemenu','fontfamilymenu',
	...
	var config = {
		...
		*/
		cBase : {
			/**
			* @type {string} ie浏览器使用需要额外加载文件的路径
			*/
			ieDir : rootDir+'src/core/base/ie/',
			/**
			* @type {boolean} 是否需要记录日志
			*/
			isLog : isLog,
			/**
			* @type {boolean} 是否需要记录错误
			*/
			isError : isError,
			/**
			* @type {boolean} 是否需要记录历史（开启撤销、重做功能）
			*/
			isHistroy : isHistory,
			/**
			* @type {array.<string>} 必须要加载的文件
			*/
			mustFile : [],
			/**
			* @type {array.<string>} 可以延时加载的文件
			*/
			delayFile : [],
			/**
			* @type {string} 语言
		...
		*/
		cEvent : {
			/**
			* @type {array.<string>} 黑名单，禁用的事件名，优先级低于白名单
			*/
			blackList : [],
			/**
			* @type {array.<string>} 白名单，启用的事件名，优先级高于黑名单
			*/
			whiteList : []
		},
		/**
		...
		*/
		cPlugin : {
			addr : rootDir+'src/core/plugin/',
			/**
			* @type {array.<string>} 黑名单，禁用的插件名，优先级低于白名单
			*/
			blackList : [],
			/**
			* @type {array.<string>} 白名单，启用的插件名，优先级高于黑名单
			*/
			whiteList : []
		},
		/**
		...
		*/
		cFilter : {
			/**
			* @type {array.<string>} 黑名单，禁用的过滤器名，优先级低于白名单
			*/
			blackList : [],
			/**
			* @type {array.<string>} 白名单，启用的过滤器名，优先级高于黑名单
			*/
		...
		*/
		cHistory : {
			/**
			* @type {boolean} 是否开启历史记录功能
			*/
			onHistory : isHistory,
			/**
			* @type {boolean} 是否开启重做功能
			*/
			onRedo : isHistory,
			/**
			* @type {boolean} 是否开启撤销功能
			*/
			onRevert : isHistory,
			/**
			* @type {number} 历史记录最多记录数目
			*/
			times : 5
		},
		/**
		...
		*/
		cError : {
			...
			*/
			onError : isError,
			/**
			* @type {string} 错误发送到服务端的地址
			*/
			errorSendAddr : '',
			/**
			* @type {number} 最长错误记录数，达到数目向服务端发送
			*/
			length : 5
		},
		/**
		...
		*/
		cLog : {
			...
			*/
			onLog : isLog,
			/**
			* @type {string} 日志发送到服务端的地址
			*/
			logSendAddr : '',
			/**
			* @type {boolean} 最长日志记录数，达到数目向服务端发送
			*/
			length : 100
		},
		/**
		* @type {object.<array>.<string>} 工具栏分类对象，存储多种预设的工具条内容及顺序
		*/
		cTools : cTools,
		/**
		* @type {array.<string> | string} 工具栏显示的工具，有顺序
		...
		*/
		toolTheme : 'default',
		cSkin : {
			addr : rootDir+'src/skin/'
		},
		/**
		* @type {boolean} 工具栏在屏幕上固定位置，
		*/
		toolFix : true,
		/**
		* @type {string} 编辑区域宽
		...
		*/
		editTheme : '',
		/**
		* @type {object.<node>} 待提交表单的textarea
		*/
		submitArea : {},
		/**
		...
		*/
		...
		/**
		* @type {object} 编辑区域的window对象，创建编辑器时赋值
		*/
		editWin : {},
		/**
		* @type {object} 编辑区域的document对象，创建编辑器时赋值
		*/
		editDom : {},
...
(function(E , $){
	var rootDir = 'http://commonjs.baike.com/dist/bkeditor/0.8.0/';
	//var rootDir = 'http://localhost/bkeditor/bke/0.8.0/';
	var param = window.location.search;
	if(param.search('debug=true')){
		//rootDir = 'http://localhost/bkeditor/dist/0.8.0/';
		rootDir = 'http://commonjs.baike.com/dist/bkeditor/0.8.0/';
	}
	var config = {
		rootDir : rootDir,
		/**
	...
	History.prototype = {
		...
		**/
		initHistory : function(config){
			if(config.cHistory.onHistory){
				this.enabled = true;
			}else{
				this.enabled = false;
			}
			this.length = config.cHistory.times;
			this.frameBody = config.editDom.body;
			this.lastHistory = {historyHtml : config.oriHtml,historySelect : ''};
			this.win = config.editWin;
		},
		/**
		 * @description
		 * 置空重做数组
	...
	var utils = {
		...
		**/
		getCurElement : function(){
			try{
				var elementList = [];
				var curSelect = E.editWin.getSelection();
				if(curSelect.type === 'None'){
					var selRange = E.editDom.createRange();
					selRange.selectNodeContents(E.editDom.body);
					selRange.collapse(false);
					curSelect.addRange(selRange);
				}
				var sRange = curSelect.getRangeAt(0);
				var curElement = sRange.startContainer;
				while(curElement.nodeName && curElement.nodeName !== 'HTML' && curElement !== E.editDom){
		...
		**/
		getNodeDom : function(node){
			...
			var tmpNode = node;
			while(tmpNode){
				if(tmpNode.nodeType === 9){
					return {dom:tmpNode,node:node};
				}else{
		...
		**/
		getSelectionText :function () {
			try{
				var sRange = E.editWin.getSelection().getRangeAt(0);
		...
		**/
		...
		getBoardContent:function (curEditor,callback) {
			var win = curEditor.config.editWin , dom = curEditor.config.editDom;
...
(function (E) {
    ...
    'labelMap':{
        ...
        'emotion':'表情', 'specharsmenu':'特殊字符', 'searchreplace':'查询替换', 'search':'查找', 'replace':'替换', 'map':'Baidu地图', 'gmap':'Google地图',
	...
	var coreCommand = {
		...
		**/
		excuteCommand : function(pcmd,pvalue,args){
			// 该过程已经在点击浏览器区域的事件中执行了
			//E.editWin = this.win;
			//E.editDom = this.dom;
			//E.curId = this.Eid;
			//E.curEditor = E.editorList[this.Eid];
			var self = this;
...
(function(E){
	...
	* 编辑器实例关联的事件基类
	* @constructor
	* @param {object} config 事件配置参数
	**/
	function EditorEvent(config){
		try{
			this.editWin = config.editWin;
			this.editDom = config.editDom;
			this.Eid = config.id;
			this.curEditor = E.editorList[this.Eid];
			this.customEvents = {};
			this.editAreaEvents = {};
			this.blackList = config.cEvent.blackList;
			this.eventConfig = {};
...
(function(E){
	...
	* 编辑器实例关联的过滤器基类
	* @constructor
	* @param {object} config 过滤器配置参数
	**/
	function EditorFilter(config){
		try{
			this.editWin = config.editWin;
			this.editDom = config.editDom;
			this.Eid = config.Eid;
			this.curEditor = config.curEditor;
			this.enableList = this.configFilter(config);
			this.exeList = {};
			this.refreshList();
		}catch(ex){
...
(function(E){
	...
	for(var tmpCmd in cmdConfig.pluginCommand){
		...
		allList[objCmd.cmd] = {htmlCmd : tmpCmd,isEnable : true,scriptAddr : E.config.cPlugin.addr+objCmd.cmd+'.plugin.js'};
	}
	/*
	var allList = {
		'forecolormenu' : {
		...
		'backcolormenu' : {
			isEnable : true,
			scriptAddr : './core/plugin/backcolor.plugin.js'
		},
		'fontsizemenu' : {
			isEnable : true,
			scriptAddr : './core/plugin/fontsize.plugin.js'
		},
		'fontfamilymenu' : {
			isEnable : true,
			scriptAddr : './core/plugin/fontfamily.plugin.js'
		},

		'baikelink' : {
			isEnable : true,
			scriptAddr : './core/plugin/baikelink.plugin.js'
		},
		'hn' : {
			isEnable : true,
			scriptAddr : './core/plugin/hn.plugin.js'
		},
		'imageplugin' : {
			isEnable : true,
			scriptAddr : './core/plugin/image.plugin.js'
		},
		'elementplugin' : {
			isEnable : true,
			scriptAddr : './core/plugin/element.plugin.js'
		},
		'iconplugin' : {
			isEnable : true,
			scriptAddr : './core/plugin/icon.plugin.js'
		},
		'redoplugin' : {
			isEnable : true,
			scriptAddr : './core/plugin/redo.plugin.js'
		},
		'revertplugin' : {
			isEnable : true,
			scriptAddr : './core/plugin/revert.plugin.js'
		}
	};*/
	...
	**/
	function Plugin(attr){
		...
		if(attr){
			...
			Plugin.allList[this.id].title = E.getLang(this.id);
			Plugin.allList[this.id].isEnable = this.isEnable;
			this.htmlCmd = Plugin.allList[this.id].htmlCmd;
	...
	Plugin.prototype = {
		...
		**/
		...
		showDialog: function(editor) {
			...
			//E.utils.loadDialog(self.id, E.config.cUi.addr+ self.id +'/', function(){
				E.dialog.open({
					id: self.ui,
					editor: editor.Eid,
					title: self.title,
					...
					ok: function() {
						if ( self.check() ) {
							E.dialog.revertSelection(self.id);
							E.command(self.ui);
						} else {
							return false;
						}
		...
		**/
		...
		loadUiTpl: function(callback) {
			E.utils.loadDialog(this.id, E.config.cUi.addr+this.id+'/', callback);
		},
		
		clicked: function( on, id ) {
			id = id || this.id;
			var o = $('#'+E.curId).find('.bke-toolbar');
...
(function(E,$){
	...
	for(var tmpCmd in cmdConfig.uiCommand){
		...
		allList[objCmd.cmd] = {isEnable : true,scriptAddr : E.config.cUi.addr+shortId+'/'+shortId+'.dialog.js'};
	}/*
	var allList = {
		'colorPanel' : {
			id : 'colorPanel',
	...
	ArtDialog.prototype = {
		...
		**/
		init : function(attr){
			...
			newDialog.DOM.dialog.addClass('bke-dialog');
			newDialog.DOM.dialog.attr('id',attr.id);
			return newDialog;
		...
		**/
		open : function(attr,args){
			oriRange = E.utils.getSelectionRange(E.editWin);
			if($.dialog.get(attr.id)){
				$.dialog.get(attr.id).DOM.dialog.attr('editor',attr.editor);
			}else{
				var newDialog = this.init(attr);
				newDialog.DOM.dialog.attr('editor',attr.editor);
			}
			if(typeof args !== 'undefined'){
				var uiId = attr.ui ? attr.ui : attr.id;
				E.utils.execUi(uiId,'setValues',args);
			}
		},
		/**
		* @description
		...
		**/
		...
		revertSelection : function(){
			if(oriRange){
				E.utils.setSelectionRange(E.editWin,oriRange);
			}
		}
	};
...
var Menu = {
	...
	_setEvent: function(menu) {
		...
		menu.delegate('.bke-menu-item', 'mouseenter', function(){
	...
	Paragraph.prototype = {
		...
		*/
		init : function(win,dom){
			...
			this.mainSelection = win.getSelection();
			/*表格选中情况区分多个带改变范围*/
			var selectedCell = $(E.editDom).find('.'+ E.curEditor.config.selectCellClass);
			var selectedTable = $(E.editDom).find('.'+ E.curEditor.config.selectTableClass);
	...
	Text.prototype = {
		...
		*/
		changeText : function(styleType,styleValue){
			if(!styleValue){
				styleValue = this.judgeValue(styleType);
			}
			if(this.mainRange.collapsed){
				//设置样式
		...
		*/
        _setTag : function(queryRange,styleType){
            ...
            if(queryNode.nodeType === 3 && /\S/.test(queryNode.nodeValue)){//Text节点
                //创建空节点
				var selText = this.document.createTextNode('\u200B');
...
(function(E, $){
	...
	E.addEvent({
		name : 'autoHeight',
		type : ['complete', 'afterCommand', 'keyup'],
		fn : function(arg){
			setTimeout(function(){
	...
	$(window).scroll(function(){
		fixedToolbar(E.curEditor);
	});
	
	// 记录工具栏距离顶部的距离
	function setToolbarTop(editor){
	...
	E.addEvent({
		...
		type : ['complete'],
		fn : function(arg){
			setTimeout(function(){setToolbarTop(E.curEditor);}, 0);
		}
	});
	
	$(window).resize(function(){
		setToolbarTop(E.curEditor);
	});
	...
	function fixedToolbar( editor ){
		...
		maxScroll = o.height() + o.offset().top - 30;
...
function intable(menulist, $table) {
	var tmenu = {
		...
		submenu: [
			...
			{},
			{name:'-'},
			{name:'合并单元格', icon:'bke-', cmd:'combinecell'},
			{name:'向右合并', icon:'bke-RowMergeAfter', cmd:'combinerowafter'},
			{name:'向下合并', icon:'bke-ColumnMergeAfter', cmd:'combinecolafter'},
			{name:'-'},
			{name:'表格属性', icon:'bke-TableProps', cmd:'tableprops'}
		]};
	
	if( $table.find('caption').length ){
		tmenu.submenu[7] = {name:'删除表格名称', cmd:'combinecell'};
	} else {
		tmenu.submenu[7] = {name:'插入表格名称', cmd:'combinecell'};
	}
	
	if( $table.find('th').length ){
		tmenu.submenu[8] = {name:'删除表格标题行', cmd:'combinecell'};
	} else {
		tmenu.submenu[8] = {name:'插入表格标题行', cmd:'combinecell'};
	}
	menulist.push(tmenu);
...
(function(E,$){
	...
	**/
	function _appendA(o){console.log();
		...
		o[method]('<a href="bke: '+ cmd +'" onclick="return false" onmouseup="return false" title="'+title+'">'+text+'</a>');
	}	
	/****************************************************************************/
	/** 编辑器执行流程时相关事件                                               **/
	/****************************************************************************/
	...
	E.addEvent({
		...
		type : ['beforeCommand'],
		fn : function(arg){
			var commandName = arg.commandName;
			E.toolbar.hidePanel(commandName);
		}
	});
	// 在命令执行之前记录历史记录
	E.addEvent({
	...
	**/
	E.addEvent({
		...
		fn : function(arg){
			...
			$('.bke-icon, .bke-caret').each(function(){
				_appendA($(this));
			});
			//根据配置cTools让工具栏显示隐藏
			var tools = [];
			...
			$('.bke-FontSize, .bke-Font').each(function(){
				_appendA($(this));
			});
			//根据配置cTools调整工具栏显示顺序
			/*
...
(function(E,$){
	...
	**/
	E.listenEditarea = function(curEditor){
		var $dom = $(curEditor.config.editDom);
		...
		$dom.bind('keydown',function(e){
			...
			alert(2);return false;
				...
				//alert(3);
			}else if (e.ctrlKey && e.which === 88){//ctrl+v
				//alert(4);
			}else if (e.ctrlKey && e.which === 86){//ctrl+x
				//alert(5);
			}else if (e.ctrlKey && e.which === 90){//ctrl+z
			alert(6);return false;
			...
			alert(11);return false;
				...
				setTimeout(function(){
					E.curEditor.baseHistory.recordHistory();
					E.utils.execPlugin('element','click');
					E.utils.execPlugin('icon','click');
				},0);
		...
		$dom.bind('mouseenter',function(e){
			...
			E.editWin = curEditor.config.editWin;
			E.editDom = curEditor.config.editDom;
	...
	// 再点击编辑区域是，根据光标位置改变元素标签导航，改变图标样式
	E.addEvent({
		name : 'changeState',
		type : ['mouseup'],
		area : 'editArea',
	...
	E.listenEditareaExt = function(curEditor){
		...
		$dom.delegate('td,th', 'mousedown', function(e){
			...
			if(e.which === 1){
				...
				//绑定选中事件（鼠标移动事件）
				$cells.not($(e.target)).bind('mousemove',function(e2){
					var $target = $(e2.target), $table = $target.closest('table');
					//选中
					E.utils.removeSelection(E.editWin);
				...
				$cells.bind('mouseenter',function(e2){
					$cells.removeClass(selectCellClass);
					if(E.editWin.getSelection().type === 'None'){
						/*当拖动时移出编辑器内容区时，会产生编辑器内容区域无选中区*/
						$cells.unbind('mouseenter mousemove');
					}else{
						E.utils.setCursor(E.editWin,e.target,true);
					}
		...
		$dom.delegate('body', 'mousedown', function(e){
			...
			// 解除单元格的事件绑定
			// this = body 元素
			$(this).find('td,th').unbind('mouseenter mousemove mouseup');
...
(function(E,$){
	...
	**/
	$dom.delegate('.bke-toolbar', 'mousedown', function(e){
		...
		E.editWin = E.editorList[Eid].config.editWin;
		E.editDom = E.editorList[Eid].config.editDom;
		E.editWin.focus();
		return false;
	});
		...
		while(queryNode){
			...
			while(queryNode.nextSibling && queryNode.tagName === queryNode.nextSibling.tagName && DTD.$font[queryNode.tagName] === 1){
				if(queryNode.tagName === 'SPAN'){
					var len = queryNode.style.length;
					for(var i=0;i<len;i++){
				...
				if(selRange){
					...
					if(selRange.startOffset > outIndex){
						startSel.offset = outIndex;
						selIn = true;
					}
					if(selRange.endOffset > outIndex){
...
(function(E){
	...
	var func = function(win,node){
		...
		while(queryNode){
			...
			if(queryNode.children.length == 0 && queryNode.innerText.length === 0 && queryNode !== selNode && DTD.$block[queryNode.nodeName] !== 1){
				var cutRange = dom.createRange();
...
(function(E ,$){
	...
	**/
	...
	var iconList = {
		/**
		* @type {object} 用浏览器判断状态的图标
		**
		browserChecked : {
			Bold : {tag:'strong',state:'_off'},
			Italic : {tag:'em',state:'_off'},
			SuperScript : {tag:'sup',state:'_off'},
			SubScript : {tag:'sub',state:'_off'},
			InsertUnorderedList : {tag:'ul',state:'_off'},
			InsertOrderedList : {tag:'ol',state:'_off'},
			StrikeThrough : {style:'text-decoration',value:'line-through',state:'_off'},
			Underline : {style:'text-decoration',value:'underline',state:'_off'}
		},
		/**
		* @type {object} 自定义判断状态的图标
		**
		customChecked : {
			JustifyLeft : {style:'text-align',value:'left',state:'_on'},
			JustifyCenter : {style:'text-align',value:'center',state:'_off'},
			JustifyRight : {style:'text-align',value:'right',state:'_off'},
			H2 : {tag:'h2',state:'_off'},
			H3 : {tag:'h3',state:'_off'}
		},
		/**
		* @type {object} 用浏览器取值的图标
		**
		browserValue : {

		},
		/**
		* @type {object} 自定义取值的图标
		**
		customValue : {
			ForeColor : {style:'color',state:'#000000'},
			BackColor : {style:'background-color',state:'#FFFFFF'},
			FontSize : {style:'font-size',state:E.lang['fontsize']},
			FontFamily : {style:'font-family',state:E.lang['fontfamily']}
		}
	};*/
	E.addPlugin({
		id : 'icon',
			...
			//try{
				...
				**/
				for(cmdName in iconList.browserChecked){
					if(E.editDom.queryCommandState(cmdName) === true){
						iconState[cmdName] = '_on';
					}else{
						iconState[cmdName] = '_off';
					}
				...
				**/
				for(cmdName in iconList.browserValue){
					if(E.editDom.queryCommandValue(cmdName)){
						iconState[cmdName] = E.editDom.queryCommandValue(cmdName);
				 ...
				 * @description 判断表格图标
				 **/
				var selectedTableLen = $(E.editDom).find('.'+ E.curEditor.config.selectTableClass).length;
				var selectedCellLen = $(E.editDom).find('.'+ E.curEditor.config.selectCellClass).length ;
				...
				* */
				o.find('.bke-btn').not(':has(.bke-submenu)').removeClass('bke-checked');
				o.find('.bke-btn').removeClass('bke-disabled');
