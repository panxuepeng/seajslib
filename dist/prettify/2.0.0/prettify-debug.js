...
(function() {
    ...
    */
    function sourceDecorator(options) {
       ...
       */
            ...
            // comments.
            "/(?=[^/*" + regexExcls + "])" + "(?:[^/\\x5B\\x5C" + regexExcls + "]" + "|\\x5C" + regexAny + "|\\x5B(?:[^\\x5C\\x5D" + regexExcls + "]" + "|\\x5C" + regexAny + ")*(?:\\x5D|$))+" + "/";
        ...
        [ PR_LITERAL, /^@[a-z_$][a-z_$@0-9]*/i, null ], [ PR_TYPE, /^(?:[@_]?[A-Z]+[a-z][A-Za-z_$@0-9]*|\w+_t\b)/, null ], [ PR_PLAIN, /^[a-z_$][a-z_$@0-9]*/i, null ], [ PR_LITERAL, new RegExp("^(?:" + "0x[a-f0-9]+" + "|(?:\\d(?:_\\d+)*\\d*(?:\\.\\d*)?|\\.\\d\\+)" + "(?:e[+\\-]?\\d+)?" + ")" + "[a-z]*", "i"), null, "0123456789" ], // Don't treat escaped quotes in bash as starting strings.
        // See issue 144.
   ...
   */
    ...
    define("prettify/2.0.0/prettify-debug", [], function(require, exports) {
        require.async("./prettify-debug.css");
        PR.autorun = function() {
            jQuery("pre[data-toggle=prettify]").addClass("prettyprint linenums");
            PR.prettyPrint();
        };
        return PR;
    });
