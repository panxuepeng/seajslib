...
*/
(function($) {
    define("selectlist/0.1.0/selectlist-debug", [], function(require, exports) {});
    $.fn.selectList = function(data, config) {
        var self = this, selectList = new SelectList(self[0], data, config);
        return self;
    };
    /*
...
*/
    fn.init = function(data) {
        var self = this, tmp_data = self.tmp_data, tmp = null, config = self.config, parent_id = 0, load_select = null;
        //非数组或者空数组退出
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (tmp_data[tmp.parent_id] instanceof Array) {
                ...
                if (config.select_text.length) {
                    //默认项id为空字符串
                    tmp_data[tmp.parent_id] = [ {
                        id: "",
                        name: config.select_text
                    } ];
                    tmp_data[tmp.parent_id].push(tmp);
                } else {
...
*/
    fn.load_select = function(parent_id) {
        var self = this, tmp_data = self.tmp_data, load_data = tmp_data[parent_id], box = self.box, html = [], tmp = null, config = self.config, parent_obj = box.find("select[parent_id=" + parent_id + "]");
...
*/
    fn.change = function(obj) {
        var self = this, parent_id = obj.attr("parent_id"), id = obj.val(), box = self.box, load_select = null, config = self.config;
...
*/
    fn.set_select = function(selected_id) {
        var self = this, data_map = self.data_map, obj = selected_id ? data_map[selected_id] : self.select_obj, objs = [], tmp = null, box = self.box;
...
*/
    fn.clear_next = function(parent_id) {
        var self = this, box = self.box, now_select = box.find("select[parent_id=" + parent_id + "]");
