...
(function($, window, undefined) {
    ...
    */
        content: function(message) {
            var prev, next, parent, display, that = this, $content = this.dom.content, content = $content[0];
            if (this._elemBack) {
                this._elemBack();
    ...
    */
        ...
        /** @inner 位置居中 */
        position: function() {
            var dom = this.dom, wrap = dom.wrap[0], $window = dom.window, $document = dom.document, fixed = this.config.fixed, dl = fixed ? 0 : $document.scrollLeft(), dt = fixed ? 0 : $document.scrollTop(), ww = $window.width(), wh = $window.height(), ow = wrap.offsetWidth, oh = wrap.offsetHeight, left = (ww - ow) / 2 + dl, top = (wh - oh) * 382 / 1e3 + dt, // 黄金比例
            style = wrap.style;
    ...
    */
        ...
        close: function() {
            ...
            var dom = this.dom, $wrap = dom.wrap, list = artDialog.list, beforeunload = this.config.beforeunload;
 ...
 */
    ...
    $.artDialog = artDialog;
    $(document).delegate("a[data-toggle=artdialog]", "click.artdialog", function(e) {
        ...
        if (url.indexOf("#") > -1) {
            ...
            // 所以，需要修正一下
            url = "#" + url.split("#")[1];
            config.content = $(url).html() || $(url).data("htmltpl");
            $(url).empty().data("htmltpl", config.content);
        } else {
    ...
    define("artdialog/5.0.3/artdialog-debug", [], function(require, exports) {
        require.async("./skins/" + (_isIE6 ? "default" : "green") + ".css");
        return jQuery.artDialog;
    });
