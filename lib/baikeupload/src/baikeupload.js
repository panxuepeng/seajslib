...
define(function(require, exports){
	var options = {},
		defaults = {
			url: '',
			uploading: function(){},
			success: function( imgUrl ){alert(imgUrl)},
	...
	$.fn.baikeUpload = function( option ) {
		...
		this.each(function(index, el){
			var id = '_' + Math.round(Math.random() * 100000);
			options[id] = option;
			option.id = id;
			init(option);
			option.btn = $(el);
	...
	function init(option) {
		var id = option.id,
			iframeid = option.iframeid = '__baikeUploadIframe'+id,
			formid = 'baikeUploadForm'+id,
			html = '<div style="position:absolute;left:-1000px;width:1px;height:1px;overflow:hidden;">\
		<form id="'+formid+'" action="'+option.url+'" method="post" target="'+iframeid+'" enctype="multipart/form-data" >\
		<input name="img_src_action" value="baike_logo_upload"/>\
		<input name="picWidthHeight" value="2"/>\
		<input name="picAlign" value="center"/>\
		<input name="picAlt" value="1"/>\
