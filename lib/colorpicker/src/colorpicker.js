...
(function($){
	 function getColorPicker(noColorText){
		...
		var html = '<div id="edui-colorpicker" class="edui-colorpicker" style="display:none;background-color:#FFF;position:absolute;">' +
		...
		for (var i=0; i<COLORS.length; i++) {
			...
			html += i<70 ? '<td style="padding: 0 2px;"><a hidefocus  title="'+COLORS[i]+'"  href="javascript:" class="edui-box edui-colorpicker-colorcell"' +
						' value="#'+ COLORS[i] +'"'+
	...
	var dom = $(document),
		colorPanel = getColorPicker('清除颜色'),
		targetInput = null;
	...
	setTimeout(function(){
		...
		dom.delegate('.edui-colorpicker-colorcell', 'mouseenter', function (){
			var o = $(this);
			preview.css({'background-color':o.attr('value')});
		});
