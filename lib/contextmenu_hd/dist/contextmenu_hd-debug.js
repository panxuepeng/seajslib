 ...
 */
(function($) {
    var dom = $(document), setTimeVal = null, menuLeft = 0, menuTop = 0;
    var Menu = {
        //创建菜单
        create: function(menu) {
            var htmlstr = '<div class="hd-menu">					<div class="hd-shadow hd-default"></div>					<div class="hd-menu-body">';
            for (var k in menu) {
                var submenu = divmore = "";
                ...
                if (k == "sep1") {
                    htmlstr += '<div class="hd-menu-item hd-menu-separator">						<div class="hd-menu-separator-inner"></div>						</div>';
                } else {
                    var disabledClass = "", icon = "";
                    if (menu[k].disabled) {
                    ...
                    htmlstr += '<div class="hd-menu-item " evt="' + menu[k]["event"] + '">								<div class="hd-menu-icon' + icon + disabledClass + '"></div>								<div class="hd-menu-label">' + menu[k].name + submenu + "</div>" + divmore + "</div>";
                }
            }
            htmlstr += "</div></div>";
            return htmlstr;
        ...
        setPosition: function(event) {
            var clientX = event.clientX, clientY = event.clientY, redge = $(window).width() - clientX, bedge = $(window).height() - clientY;
            var menu = $(".hd-menu"), menu_width = menu.width(), menu_height = menu.outerHeight();
            if (redge < menu_width) {
    ...
    dom.delegate(".hd-menu-item", "mouseenter", function(e) {
        ...
        if (submenu.size()) {
            ...
            var pos = $(this).position(), swidth = submenu.outerWidth(), sheight = submenu.outerHeight();
...
define("contextmenu_hd/0.1.0/contextmenu_hd-debug", [], function(require, exports) {
    require.async("./contextmenu_hd-debug.css");
    return $;
});