...
var IE = window.VBArray,
	...
	DIALOG_1_0 = false; // 是否dialog1.0版本的配置文件，如果是的话需要做一些转换
...
function setOption(op){
	...
	if( op.overlay ){
		if(op.styleOverlay){
			...
			// 设置设置 op.overlay = true 不需要出发1.0模式
			// 2012-07-05
			//DIALOG_1_0 = true;
		}
	}
	...
	if( op.closeImg === false ){
		op.style.close = {display:'none'};
		DIALOG_1_0 = true;
	}else if( DIALOG_1_0 ){
		op.style.close = {display:'block'};
	}
	
	if( DIALOG_1_0 ){
		if( op.style.ok.display == 'none' && op.style.cancel.display == 'none' ){
			op.style.bottom = {display:'none'};
		}
...
$.dialog = function(option){
	...
	if( dialog ){
		...
		config.zIndex+=1;
		dialog.panel.css({zIndex: config.zIndex});
...
//	fixedSize: false, // 窗口大小自适应内容，并自动根据内容重新定位窗口
	...
	autoClose: 0, // 自动关闭
	model:'default',//default together alone
...
$.extend(Dialog, {
	...
	init: function(){
		...
		var style = [];
		// 添加预定义的皮肤
		style.push('.default .jqd-inner-wrap{border:2px solid #148ea4;border-radius:6px;box-shadow:0 0 8px #148ea4;}\
.default .jqd-title{background-color:#148ea4;border-bottom:1px solid #E5E5E5;color:#FFFFFF;font-weight:bold;}\
.default .jqd-close{width:18px;height:20px;top:2px;right:5px;}\
.default .jqd-button button{width:80px;height:27px;cursor: pointer;font-size:12px;line-height:100%;_line-height:26px;}');

		style.push('.noborder .jqd-inner-wrap{}\
.noborder .jqd-title{display:none;}\
.noborder .jqd-content{padding:0;}\
.noborder .jqd-close{display:none;}\
.noborder .jqd-bottom-pad{height:0;}\
.noborder .jqd-bottom{display:none;}');

		style.push('.bluebox .jqd-bg{background:#000; filter:alpha(opacity=30);opacity:0.3; width:100%; height:100% !important; _height:1000px;position:absolute; z-index:-1; zoom:1;}\
.bluebox .jqd-inner-wrap{}\
.bluebox .jqd-title{background-color:#009DF0;color:#FFF;font-weight:bold;}\
.bluebox .jqd-close{top:3px;right:5px;}\
.bluebox .jqd-button button{width:80px;height:27px;cursor: pointer;font-size:12px;line-height:100%;_line-height:26px;}');
		
		style.push('.greybox .jqd-outter-wrap{}\
.greybox .jqd-bg{background:#000; filter:alpha(opacity=8);opacity:0.08; width:100%; height:100% !important; _height:1000px;position:absolute; z-index:-1; zoom:1;}\
.greybox .jqd-inner-wrap{border:1px solid #A5A5A5;border-radius:2px;box-shadow:0;}\
.greybox .jqd-title{border:0; height:30px;font-size:14px;line-height:30px;color:#87C7DB;background-color:#F3F3F3;}\
.greybox .jqd-close{width:10px;height:10px;top:10px;right:14px;background:url(http://www.huimg.cn/xiaobaike/images/baike2.0/form.png) -42px -108px;text-indent:2em;}\
.greybox .jqd-button button{width:80px;height:27px;cursor: pointer;font-size:12px;line-height:100%;_line-height:26px;}');

		style.push('.xiaobaike .jqd-outter-wrap{}\
.xiaobaike .jqd-bg{background:#000; filter:alpha(opacity=8);opacity:0.08; width:100%; height:100% !important; _height:1000px;position:absolute; z-index:-1; zoom:1;}\
.xiaobaike .jqd-inner-wrap{border:1px solid #A5A5A5;border-radius:2px;box-shadow:0;}\
.xiaobaike .jqd-title{height:20px;line-height:20px;font-size:18px;color:#87C7DB;padding:11px 0 1px 15px;font-family:\\5FAE\\8F6F\\96C5\\9ED1;}\
.xiaobaike .jqd-content{padding-top:5px;}\
.xiaobaike .jqd-close{width:10px;height:10px;top:15px;right:14px;background:url(http://www.huimg.cn/xiaobaike/images/baike2.0/form.png) -42px -108px;text-indent:2em;}\
.xiaobaike .jqd-button button{width:80px;height:27px;cursor: pointer;font-size:12px;line-height:100%;_line-height:26px;}');
		
		style.push('.hudong .jqd-title, .hudong .jqd-close, .hudong .jqd-button, .hudong .jqd-button button{background:url(http://www.huimg.cn/lib/dialog/bg_minbox.png) no-repeat;}\
.hudong .jqd-title{background-repeat:repeat-x;height:30px;border-top:1px solid #69B2FC; font-weight:bold; color:#FFF; line-height:30px;}\
.hudong .jqd-bg{background:#000; filter:alpha(opacity=20);opacity:0.2; width:100%; height:100% !important; _height:1000px;position:absolute; z-index:-1; zoom:1;}\
.hudong .jqd-content{padding:2px 20px;}\
.hudong .jqd-bottom{height:25px;padding:5px 0 8px; text-align:center;}\
.hudong .jqd-close{width:16px;height:16px;top:7px;right:7px; background-position:0 -218px;text-indent:2em;}\
.hudong .jqd-button{display:inline-block;  margin:0 4px 0 3px; background-position:0 -32px;font-size:12px;}\
.hudong .jqd-button button{margin:0; cursor:pointer;height:31px; padding:0 14px; *padding:0 4px; border:0 none; background-position:right -63px;font-weight:bold;color:#FFF;line-height:31px;}\
.hudong .jqd-reg{background-position:0 -94px;}\
.hudong .jqd-reg button{background-position:right -125px; color:#2375CB;}\
.hudong .jqd-cancel{background-position:0 -156px;}\
.hudong .jqd-cancel button{background-position:right -187px; color:#2C2C2C;}');
	...
	// 将窗口默认样式添加到页面
	setStyle: function(){
		var style = '.jquery-dialog{position:absolute;z-index:1000;font-size:12px;overflow:hidden;}\
.jquery-dialog td{padding:0px;}\
.jqd-outter-wrap{padding:4px;}\
.jqd-outter-wrap table{margin:0;}\
.jqd-bg{border-radius:4px;}\
.jqd-inner-wrap{position:relative;background-color:#FFF;}\
.jqd-title{padding:0px 10px;font-size:16px;color:#FFFFFF;cursor:default;height:30px;line-height:30px;}\
.jqd-close{position:absolute;top:5px;right:20px;font-size:24px;color:#FFFFFF;cursor:pointer;width:18px;height:18px;overflow:hidden;font-weight:bold;font-family: tahoma;line-height:20px;}\
.jqd-content{padding:0px 10px;}\
.jqd-text{padding:10px;text-align:left;line-height:20px;font-size:14px;}\
.jqd-bottom{text-align:right;height:25px;padding-right:20px;}\
.jqd-bottom-pad{height:8px;}\
.jqd-cancel {margin-left:10px;}\
.jqd-status,.jqd-success, .jqd-error{position:absolute;width:200px;bottom:9px;background-color:#F5F5F5;color:#666;line-height:22px;border:1px dashed #CCC;border-radius:5px;left:20px;padding:0 10px;display:none;}\
.jqd-success{color:green;}\
.jqd-error{color:red;}';
	...
	*/
	getHtml: function( option ){
		...
		// 浏览器兼容需要使用table标签
		return '<div class="jqd-bg"></div><div class="jqd-outter-wrap">\
	<table border="0" cellspacing="0" cellpadding="0"><tbody><tr><td>\
	<div class="jqd-inner-wrap">\
	<div class="jqd-title">'+o.title+'</div><a class="jqd-close" name="close" title="关闭">×</a>\
	<div class="jqd-content" id="jqd-content-'+o.id+'" style="'+style+'"></div>\
	<div class="jqd-bottom">\
	<span class="jqd-ok jqd-button"><button name="ok" type="submit" >'+o.textOk+'</button></span>\
	<span class="jqd-cancel jqd-button"><button name="close">'+o.textCancel+'</button></span>\
	</div>\
	<div class="jqd-bottom-pad"></div>\
	<div class="jqd-status" id="jqd-status-'+o.id+'"></div>\
	...
	bindEvent: function(self){
		...
		if( IE6 && op.fixed ){
			win.bind('scroll', function(){
				// 连续的滚屏停止后执行一次滚动操作
				clearTimeout(timeId);
				timeId = setTimeout(function(){if(!self.closed)Dialog.scroll(self)}, 200);
			});
		}
		...
		panel.mousedown(function(e){
			...
			if( !op.zIndex && $.dialog.self != self ){
				config.zIndex += 2;
				panel.css({zIndex: config.zIndex});
				$.dialog.self = self;
			}
		});
	},
	...
	scroll: function(self){
		var op = self.option,
			top = dom.scrollTop(),
			winH = win.height(),
			dialogH = self.panel.outerHeight();
		...
		if( winH > dialogH ){
			...
			self.panel.animate({top: top}, 200);
		}
	},
	...
	getPostion: function(self){
		var o = self.panel,
			...
			dialogW = o.outerWidth(), dialogH = o.outerHeight(),
			pos={},
	...
	resetFocus: function(self){
		var o = self.panel,
			option = self.option,
			focusStart = option.focusInput == 'start'? true: false;
...
Dialog.prototype = {
	...
	show: function(){
		...
		if( self.completed && option.type !== 'iframe' ){
			setTimeout(function(){self.execCallback()}, 0);
		}
		return self;
	},
	...
	setContent: function( content, type ){
		...
		switch(type){
			...
			case 'iframe':
				url = content;
				content = "<iframe id='"+id+"_iframe' name='"+id+"_iframe' border='0' width='"+ op.width+"' height='"+op.height+"' frameborder='no' "
		...
		if( !html || !/<\/(div|table|dl|ul|ol|iframe)>/i.test( html ) ){
			html = '<table border="0" cellspacing="0" cellpadding="0" width="'
			 + op.width+'" height="'+op.height+'">'
			 + '<tbody><tr><td class="jqd-text" style="text-align:'+op.align+'; vertical-align:'+op.valign+'">'
			 + html
			 + '</td></tr></tbody></table>';
		}
		...
		if( 'img' === type ){
			...
			if( self.completed ){
				...
				if( 'ajax' === op.type ){
					setTimeout(function(){self.execCallback(html)}, 0);
				}else if( 'img' === op.type ){
					setTimeout(function(){self.execCallback()}, 0);
				}
			}
	...
	resetHeight: function(){
		...
		if( op.fixed && op.skin != 'noborder' ){
			var maxHeight =  win.height(),
				contentElement = o.find('.jqd-content');
			...
			if( o.height() > maxHeight ){
				contentElement.css({
					...
					width:contentElement.width(), //? 需要设置这个吗？
					display:'block',
					overflowX:'hidden',
					overflowY:'scroll'
				});
	...
	execCallback: function( html ){
		...
		if( self.completed ){
			...
			if( option['autoClose'] ){
				...
				option.autoClose = isNaN(option.autoClose)? 3000: option.autoClose;
	...
	notice: function( html ){return this.status(html)},
	success: function( html ){return this.status(html, 'success')},
	error: function( html ){return this.status(html, 'error')},
...
// 暂时改为延迟执行（2012-03-26）
// Dialog.init();
setTimeout(function(){Dialog.init();}, 0);
...
Overlay.prototype = {
	show: function(){
		...
		if( IE6 ){
			o.show().css({
				opacity: op.opacity,
				position:'absolute' // ie6 不支持fixed属性，需要使用绝对定位
			});
		}else{
			o.css({opacity:0, display:'block'});
			o.animate({opacity: op.opacity}, 200);
		}
		return self;
	},
	...
	resize: function(){
		...
		// ie6下需要修改top和margin-left
			o.css({top: dom.scrollTop(), height:win.height()});
