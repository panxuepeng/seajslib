	 ...
	 var IE6Postion = {
		...
		onright: function(option) {
			var elem = option.element,
				...
				right = o.data('right-ie6')||o.data('right')||0,
				bottom = o.data('bottom-ie6')||o.data('bottom')||0;
		...
		oncenter: function(option){
			var elem = option.element,
				...
				domWidth = $(document).width(),
				offset = o.data('offset-ie6')||o.data('offset')||0,
				top = o.data('top-ie6') || o.data('top') || 'auto',
				bottom = o.data('bottom-ie6') ||o.data('bottom') || 0;
			...
			if (option.trigger === 'auto' || option._isFlow) {
				o.css({right: 'auto', left:'50%', 'margin-left': offset});
			}
			...
			$.each(fixedList, function(i, option){
				...
				if ( option.trigger !== 'auto' ) {
					if (getScrollTop()+option.top >= option._css.offsetTop) {
						// 浮动
						if (!option._isFlow) { // 避免浮动状态下代码重复执行
							if (IE6) {
								style.position = 'absolute';
								style.removeExpression('top');
