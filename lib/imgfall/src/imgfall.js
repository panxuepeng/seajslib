...
define(function(require, exports){
	var interImage = require('./image');
	var interPanel = require('./panel');
	...
	var Fall = Backbone.Model.extend({
		defaults : {
			id:'container',                 //使用fall的div
			dataUrl:'index.php',            //获取数据的url

			panel_class:'panel',            //容器样式类
			last_panel_class:'last_panel',  //最后一列容器样式类
			per_page:20,                    //每页的数据
			panel_count:5,                  //列数
			end_distance:20,                //出发获取新数据时，滚轮距底部的距离
			page:1,                         //从第几页开始获取
			del_show:false,                 //是否显示删除图标
			inner_contain:'dl',             //容器图片容器标签，用于查找
			end_tips:''						//结尾提示
		...
		create_panel:function(n){
			...
			for(var i=0;i<n-1;i++){
				...
				panel_div = new interPanel.PanelShow({model:panel,el:$("#"+self.attributes.id)});
			}
			attr = {
				id : 'panel_'+i ,
		...
		create_pause:function(){
			//生成控制请求数据的开关
			var self = this;
			panel_div = new SwitchShow({el:$("#"+self.attributes.id)});
		},
		...
		into_panel:function(image,flag){
			...
			var showImage = new interImage.ImageShow({model:newImage,el:$("#"+panel_id)});
		},
		...
		ajax_get_image:function(page_num){
			...
			var self = this;
			$.get(self.attributes.dataUrl,{page:page_num},function(data){self.load_image(data);},"json");
		},
		start:function(){
			//开始瀑布流
			var self = this;
			...
			$(window).bind("scroll",function(){
				...
				if( $(document).scrollTop() + $(window).height() > $(document).height() - self.attributes.end_distance && !self.isPause()){
					//self.ajax_get_image(self.attributes.page);
					//改变控制继续获取图片next_page的值，使之不能继续获取
					//self.pause();
				}
			});
		},
