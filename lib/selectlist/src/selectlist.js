...
/*
	...
	id: option的value值，
	name：option显示内容，
	pid: 父级菜单项的id，没有可以为空字符串或者null[也可以不加此属性]

var testData = [{id:1,pid:0,name:'一级1'},
				{id:2,pid:0,name:'一级2'},
				{id:21,pid:1,name:'二级1'},
				{id:22,pid:1,name:'二级2'},
				{id:31,pid:21,name:'三级1'},
				{id:32,pid:21,name:'三级2'},
				{id:41,pid:31,name:'四级1'},
				{id:51,pid:41,name:'四级2'},
				{id:52,pid:41,name:'四级2'},
				{id:221,pid:2,name:'二级一'},
				{id:222,pid:2,name:'二级二'},
				{id:331,pid:222,name:'三级一'},
				{id:332,pid:222,name:'三级二'},
				{id:441,pid:332,name:'四级一'},
				{id:442,pid:332,name:'四级二'},
				{id:551,pid:442,name:'五级一'},
				{id:552,pid:442,name:'五级二'}
				];
使用方法
...
function SelectList(box,data,config){
	...
	*/
	...
	/*
		...
		{
			0:[{id:11,pid:0, name:'一级1'},{id:12,pid:0, name:'一级2'}],
			11:[{id:21,pid:0, name:'二级1'},{id:22,pid:0, name:'二级2'}],
			21:[{id:31,pid:0, name:'三级1'},{id:32,pid:0, name:'三级2'}],
		}
	*/
	this.tmp_data = {};
	/*
		...
		格式如：{
			1 : {id:1,pid:0, name:'一级1'},
			2 : {id:2,pid:0, name:'一级2'},
			3 : {id:3,pid:0, name:'三级2'}
		}
	*/
	this.data_map = {};
	/*
...
fn.init = function(data){
	...
	for(var i = 0 ; i < data.length; i++){
		tmp = data[i];
		//不提供pid 或者pid为否值则默认为一级菜单,pid用0替代
		pid = tmp.pid ? tmp.pid : pid;
		...
		if(tmp_data[tmp.pid] instanceof Array){
			tmp_data[tmp.pid].push(tmp);
		}else{
			//如果配置了默认选项则在数组开始增加该项
			if(config.select_text.length){
				//默认项id为空字符串
				tmp_data[tmp.pid] = [{id: '', name: config.select_text}]
				tmp_data[tmp.pid].push(tmp);
			}else{
				tmp_data[tmp.pid] = [tmp];
			}
		}
	}
	//初始化时显示一级列表，pid = 0
...
*/
fn.load_select = function(pid){
	var self = this, tmp_data = self.tmp_data, load_data = tmp_data[pid],
		box = self.box, html= [], tmp = null, config= self.config,
		parent_obj = box.find('select[pid='+pid+']');
	...
	if(parent_obj.length){
		...
		parent_obj = $('<select pid="'+pid+'">'+html.join('')+'</select>');
...
/*
	...
	功能：删除obj后面的select标签，如果存在id则加载该pid = id的所有列表项
*/
...
fn.set_select = function(selected_id){
	...
	box.find('select[pid]').remove();
	while(obj){
		objs.push(obj);
		obj = data_map[obj.pid];
	}
	//最后一个select需要触发chang事件
	while(objs.length){
		obj = objs.pop();
		tmp = self.load_select(obj.pid);
		tmp.val(obj.id);
		if(objs.length == 0){
			tmp.trigger('change');
		}
...
/*
	...
	@string pid : 当前select标签的pid属性值
*/
fn.clear_next = function(pid){
	var self = this, box = self.box,
		now_select = box.find('select[pid='+pid+']');
	if(now_select.length){
		while(now_select.next('select[pid]').length){
			now_select.next('select[pid]').remove();
		}
	}
}
