/*
 * slide
 * https://github.com/Administrator/slide
 *
 * Copyright (c) 2013 fanglong
	...
	Slide.prototype = {
		init: function(opt){
			...
			if (o.playtype == 'sideslip'){
				self.photoArr.each (function (i) {$(self.photoArr[i]).css('z-index',self.count-1-i).show();});
			}
			...
			self.photoArr.hover (
				...
				//鼠标移走，继续循环
				function () {self.InterVal =setInterval( function () { self.play(o.playtype);},o.timeout)}
			);
			self.navArr.find('span').hover (
				//鼠标移动到导航上，切换到当前图片
				function () {
			...
			//定义器设置，让每2秒执行一次播放函数
			self.InterVal = setInterval (function () { self.play(o.playtype);},o.timeout);
		},
		...
		slide:function () {
			var self = this,
				o = self.config,
				width = self.photoArr.outerWidth(),
				height = self.photoArr.outerHeight();
			self.current++;
			if(self.current == self.count){
				self.current = 0;
			}
			if(o.direction == 'top'){
				$(o.box).animate({scrollTop:self.current*self.height},o.speed);
			} else {
				$(o.box).animate({scrollLeft:self.current*self.width},o.speed);
			}
		...
		shadow:function () {
			var self = this,
				o = self.config;
			self.photoArr.each (function (i) {$(self.photoArr[i]).css('z-index',self.count-i).show();});
		...
		sideslip:function () {
			var self = this,
				o = self.config;
			self.photoArr.css({position:'absolute',top:0,left:0,overflow:'hidden',width:self.width,height:self.height});
			//重设略图zIndex
			self.resetIndex(self.current);
			//当前图片滑走
			$(self.photoArr[self.current]).animate({left:-self.width},self.timeout );
			self.current++;
			if(self.current == self.count){
				self.current = 0;
			}
			$(self.photoArr[self.current]).animate({zIndex:self.count-2},self.timeout);
			self.navArr.find('span').removeClass(o.current).eq(self.current).addClass(o.current);
			self.zindex = self.zindex + 1;
		},
